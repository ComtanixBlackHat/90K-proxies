    ...
    try:
        ...
        for row in rows:
            ...
            if filehand.verifyipadress(ad) is True:
                filehand.writeIntoFile(ad , "temp3.csv")
                global total_count
                total_count +=  1
            counter+=1
        counter = 2    
...
try:
    ...
    for i in range( 301 , 600):
        ...
        scrap(driver)
    filehand.remove_duplicate("/root/90/code/temp3.csv")   
    os.remove("/root/90/code/temp3.csv") 
    print("Iproyal.com page 400-600 scrapper finished . total of "+str(total_count) +" proxies scrapped") 
    # driver.quit()
    sys.exit()
except Exception as e:
    filehand.writeLog('iproyal.com page 400-600' ,  str(e))
    print (e)
    driver.quit()
    sys.exit()
finally:
