    ...
    try:
        ...
        for row in rows:
            ...
            if filehand.verifyipadress(ad) is True:
                filehand.writeIntoFile(ad , "temp2.csv")
                global total_count
...
total_count = 0
try:
    print("Iproyal.com scrapper is running from page 200-400")
    for i in range( 201, 400):
        driver.get('https://iproyal.com/free-proxy-list/?page='+str(i))
        scrap(driver)
    filehand.remove_duplicate("/root/90/code/temp2.csv")   
    os.remove("/root/90/code/temp2.csv") 
    print("Iproyal.com page 200-400 scrapper finished. total of "+str(total_count) +" proxies scrapped") 
    sys.exit()
except Exception as e:
    filehand.writeLog('iproyal.com page 200-400' ,  str(e))
    print (e)
    driver.quit()
    sys.exit()
finally:
