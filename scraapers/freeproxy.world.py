        ...
        for row in rows:
            try:    
                ...
                if filehand.verifyipadress(ad) is True:
                    filehand.writeIntoFile(ad , "tempfreeproxy1.csv")
                    global total_count
...
# 400
try:
    print("freeproxy.world scrapper is running from page 1-200")
    for i in range( 1, 200):
        driver.get('https://www.freeproxy.world/?type=&anonymity=&country=&speed=&port=&page='+str(i))
        scrap(driver)

    filehand.remove_duplicate("tempfreeproxy1.csv")   
    os.remove("/root/90/code/tempfreeproxy1.csv") 
    print("freeproxy.world  page 1-200 finished . total of "+str(total_count) +" proxies scrapped")  
    sys.exit()
except Exception as e:
    print (e)
